...
- **TASK-241: Enforce TaskForgeCommandResult return schema as invariant** — Defined TaskForgeCommandResult interface with Zod schema and all sub-interfaces. Created result builder helpers (success, blocked, failed, noop, humanRequired, doctorRequired, contextCleanup). Added command-specific validNextCommands maps for 35+ CLI commands. Created Markdown and JSON renderers with 9-section output per spec. Standard prohibited actions (5) included in every normal-agent result. 24 new tests in `command-result.test.ts`. (Core infrastructure complete; command wiring deferred to follow-up)
