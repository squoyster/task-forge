...
- **TASK-145: Remove direct task mutation from start before transaction** — `taskforge start` no longer calls `updateTaskLock`, `updateTaskStatus`, `writeTaskFile`, or `appendAgentNote` directly before the transactional claim. All mutations now happen inside `withTaskStateTransaction`, ensuring durable claim semantics.
