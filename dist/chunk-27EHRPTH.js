...
// src/util/write-command-result.ts
...
import { z } from "zod";
var ValidNextCommandSchema = z.object({
  command: z.string(),
  purpose: z.string(),
  when: z.string(),
...
var TodoMergeItemSchema = z.object({
  taskId: z.string(),
  action: z.enum(["add", "remove", "update"]),
  content: z.string()
});
var TodoMergeInstructionSchema = z.object({
  required: z.boolean().default(false),
  items: z.array(TodoMergeItemSchema).default([])
});
var ContextCleanupInstructionSchema = z.object({
  required: z.boolean().default(false),
  reason: z.string().optional(),
  actions: z.array(z.string()).default([])
});
var ProhibitedActionSchema = z.object({
  action: z.string(),
  reason: z.string()
});
var RecoveryInstructionSchema = z.object({
  required: z.boolean().default(false),
  steps: z.array(z.string()).default([]),
  createTaskBody: z.string().optional()
});
var DiagnosticItemSchema = z.object({
  level: z.enum(["info", "warn", "error"]),
  message: z.string()
});
var AuditReferenceSchema = z.object({
  taskId: z.string().optional(),
  transcriptPath: z.string().optional(),
  eventId: z.string().optional()
});
var CommandMetadataSchema = z.object({
  command: z.string(),
  timestamp: z.string(),
  duration: z.number().optional(),
  sessionId: z.string().optional()
});
var CommandStatusSchema = z.enum([
  "success",
  "blocked",
  "failed",
  "noop",
...
var CommandContextSchema = z.object({
  taskId: z.string().optional(),
  worktree: z.string().optional(),
  branch: z.string().optional()
});
var AgentPromptEnvelopeSchema = z.object({
  role: z.enum(["implementer", "reviewer", "planner", "intake", "doctor"]).default("implementer"),
  instruction: z.string().optional()
});
var TaskForgeCommandResultSchema = z.object({
  ok: z.boolean(),
...
function withError(result, error, code) {
  result.error = error;
  result.code = code;
  return result;
}
...
function createStatusSchema() {
  return z2.preprocess(
    ...
    z2.enum([STATUS.INBOX, STATUS.NEEDS_SPEC, STATUS.READY, STATUS.IN_PROGRESS, STATUS.BLOCKED, STATUS.IMPLEMENTATION_COMPLETE, STATUS.SUBMITTED, STATUS.REVIEW, STATUS.MERGE_READY, STATUS.VERIFY, STATUS.DONE, STATUS.REJECTED, STATUS.DEFERRED])
  );
}

// src/core/task.ts
var TaskPriority = z3.enum(["P0", "P1", "P2", "P3"]);
var TaskType = z3.enum([
  "Epic",
  "Feature",
  "Task",
  "Bug",
...
var RiskLevel = z3.enum(["Low", "Medium", "High"]);
var TaskStatus = z3.enum([
  STATUS.INBOX,
  STATUS.NEEDS_SPEC,
  STATUS.READY,
  STATUS.IN_PROGRESS,
...
var BlockedBy = z3.enum(["human", "agent", "bot", "unspecified"]);
var TaskSchema = z3.object({
  id: z3.string(),
  type: TaskType.default("Task"),
  status: createStatusSchema().default(STATUS.INBOX),
  priority: TaskPriority.default("P2"),
  agentRole: z3.string().optional(),
  riskLevel: RiskLevel.default("Low"),
  humanInterventionRequired: z3.boolean().default(false),
  dependsOn: z3.array(z3.string()).optional(),
  assignee: z3.string().optional(),
  claimed_at: z3.union([z3.string(), z3.date()]).optional(),
  branch: z3.string().optional(),
  worktree: z3.string().optional(),
  blocked_reason: z3.string().optional(),
  blocked_by: BlockedBy.optional(),
  blocked_since: z3.union([z3.string(), z3.date()]).optional(),
  block_category: BlockCategory.optional(),
  context_hash: z3.string().optional(),
  override_reason: z3.string().optional(),
  override_actor: z3.string().optional(),
  override_timestamp: z3.string().optional(),
  override_failed_gates: z3.array(z3.string()).optional(),
  issue: z3.number().optional(),
  pr: z3.number().optional(),
  submitted_sha: z3.string().optional(),
  submitted_at: z3.union([z3.string(), z3.date()]).optional(),
  pr_merged: z3.boolean().optional(),
  pr_head_sha: z3.string().optional(),
  pr_base_branch: z3.string().optional(),
  code_task: z3.boolean().optional()
});
