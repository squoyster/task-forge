...
function generateOpenCodeConfig(policy, audit, guard) {
  const config = {
  ...
  if (audit) {
    config.taskforge = { ...config.taskforge, audit: true };
  }
  if (guard) {
    config.taskforge = { ...config.taskforge, guard: true };
  }
  return config;
}
...
function mergeConfig(existing, generated) {
  ...
  for (const [key, value] of Object.entries(generated)) {
    ...
    if (key === "taskforge") {
      result.taskforge = { ...result.taskforge, ...value };
...
//# sourceMappingURL=chunk-MCST2LCE.js.map